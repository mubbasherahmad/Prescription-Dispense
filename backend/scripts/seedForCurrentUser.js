...
const seedForCurrentUser = async () => {
  try {
    ...
    // Find the most recent user (likely your account)
    ...
    console.log(`\nAdding prescriptions to: ${targetUser.name} (${targetUser.email})`);

    // Clear existing prescriptions for this user except the ones they manually created
    // Don't delete anything, just add the sample data
    ...
    console.log(`Successfully added ${createdPrescriptions.length} prescriptions to your account`);
    console.log(`You should now see ${createdPrescriptions.length + 1} prescriptions total (including your existing ones)`);
